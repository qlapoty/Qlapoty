                                     ...
                                     const quat_alg_t *Bpoo)
{
    ...
    ibz_t tmp;
    ec_basis_t E0_basis, B1, to_be_pushed;
    ...
    if (!ret) {
        printf("failed qlapoti (SHOULDNT HAPPEN!)\n");
    } else {
...
#if (ID2ISO_DEBUG_PRINT > 2)
    printf("used_torsion depending on theta: %d\n", used_torsion);
#endif
    ...
    ibz_pow(&two_e, &ibz_const_two, TORSION_EVEN_POWER);
    UNUSED int invertible = ibz_invmod(&(tmp), d1, &two_e);
    ...
    copy_basis(&E0_basis, &BASIS_EVEN);
    endomorphism_application_even_basis(&E0_basis, 0, &CURVE_E0, &apply_to_basis, TORSION_EVEN_POWER);
    assert(test_basis_order_twof(&E0_basis, &CURVE_E0, TORSION_EVEN_POWER));
    ...
    copy_bases_to_kernel(&ker, &B1, &BASIS_EVEN);
    ...
    ret = theta_chain_compute_and_eval_randomized(
        used_torsion, &E0E0, &ker, true, &theta_codomain, push, sizeof(push) / sizeof(*push));
    if (!ret) {
    ...
    // Solutions is always E2 thanks to theta power

...
id2iso_qlapoti_benchmarks(int bitsize, int iterations, int test)
{
    ...
    ideals = malloc((warmup + iterations) * sizeof(quat_left_ideal_t));
    ...
    randret = randret || id2iso_qlapoti_benchmarks_input_generation(ideals, bitsize, iterations + warmup);
...
int
id2iso_qlapoty_benchmarks(int bitsize, int iterations, int test)
{
    int res = 0;
    int randret = 0;
    int warmup = 100;
    ...
    printf("Running qlapoty_normeq benchmarks for " STRINGIFY(
               SQISIGN_VARIANT) " with %d iterations, inputs of bitsize %d and the %d-bit prime\n",
           iterations,
           bitsize,
           ibz_bitsize(&(QUATALG_PINFTY.p)));
    ...
    for (int iter = 0; iter < warmup; iter++) {
        res = res | !quat_qlapoty(&(mu1s[iter]),
                                  &(mu2s[iter]),
                                  &(thetas[iter]),
                                  &(smalls[iter]),
                                  &(ideals[iter]),
...
fin:;
    if (randret) {
        printf("Randomness failure in id2iso_qlapoty_normeq_benchmarks\n");
    }
    if (res) {
        printf("Tests in id2iso_qlapoty_normeq_benchmarks failed\n");
    }
...
main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (!help && strcmp(argv[i], "--qlapoti") == 0) {
            qlapoti = 1;
            continue;
        }
    ...
    if (help || invalid) {
        ...
        printf("Usage: %s [--bitsize=<bitsize>] [--iterations=<iterations>] [--qlapoti] [--tests] [--seed=<seed>]\n",
               argv[0]);
        ...
        printf("Where <iterations> is the number of iterations used for benchmarking; if not present, uses the "
               ...
               iterations);
        printf(
            "When --qlapoti is passed, the original qlapoti instead of the rarely-failing qlapoty is benchmarked \n");
    ...
    return id2iso_qlapoty_benchmarks(bitsize, iterations, tests);
}

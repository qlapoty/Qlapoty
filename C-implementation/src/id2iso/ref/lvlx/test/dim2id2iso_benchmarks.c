...
void
tate_basis(fp2_t *out, int e, const ec_basis_t *basis, const ec_curve_t *curve)
{
    ibz_vec_2_t tmp;
    ec_point_t wi;
    ec_curve_t ec;
    ...
    copy_curve(&ec, curve);
    reduced_tate(out, e, &basis->P, &basis->Q, &wi, &ec);
                             ...
                             int point_order_bits)
{
    int res;
    fp2_t e1, e2, cmp;
    tate_basis(&e1, point_order_bits, input, domain);
    tate_basis(&e2, point_order_bits, output, codomain);

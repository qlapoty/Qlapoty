...
quat_test_gaussian_mul()
{
    ...
    ibz_init(&tmp);
    for (int i = 0; i < 10; i++) {
        ibz_rand_interval_bits(&a[0], 10);
        ibz_rand_interval_bits(&b[0], 10);
        ibz_rand_interval_bits(&a[1], 10);
        ibz_rand_interval_bits(&b[1], 10);
        ...
        ibz_add(&c[1], &tmp, &c[1]);
        ibz_copy(&c[0], &sum);
        res = res || !(ibz_cmp(&c[0], &g[0]) == 0);
        res = res || !(ibz_cmp(&c[1], &g[1]) == 0);
    }
    if (res) {
        printf("Quaternion unit test gaussian_mul failed\n");
    }
    ibz_vec_2_finalize(&a);
    ibz_vec_2_finalize(&b);
    ibz_vec_2_finalize(&g);
...
// void quat_qlapoty_gen_to_dim2_lattice(ibz_mat_2x2_t *lat, ibz_vec_2_t *target, const quat_alg_elem_t *gen, const
// ibz_t *N);

// void quat_qlapoty_get_short_basis(ibz_mat_2x2_t *basis, ibz_vec_2_t *target, const ibz_vec_2_t *st, const
// quat_alg_elem_t *alpha0, const ibz_t *N);

// int quat_qlapoty_initialize_bounds(ibz_vec_2_t *ab,  ibz_vec_2_t *omega,  quat_alg_elem_t *gen,  uint32_t
// *f_and_big_interval,  const ibz_t *N,  const ibz_mat_2x2_t *Li,  const quat_alg_t *alg,  const ibz_t *twoe);
int
quat_test_qlapoty_initialize_bounds()
{
    int res = 0;
    uint32_t fbi[2] = { 0, 0 };
    ibz_vec_4_t coeffs;
    ibz_vec_2_t ab, omega, omega_cmp;
    ibz_mat_2x2_t Li;
    quat_left_ideal_t small;
    quat_alg_t alg;
    quat_alg_elem_t gen, gen_cp;
    ibz_t twoe, tmp, r, q, n_gen, fivef;

    ibz_vec_4_init(&coeffs);
    ibz_vec_2_init(&ab);
    ibz_vec_2_init(&omega);
    ibz_vec_2_init(&omega_cmp);
    ibz_mat_2x2_init(&Li);
    quat_left_ideal_init(&small);
    quat_alg_elem_init(&gen);
    quat_alg_elem_init(&gen_cp);
    ibz_init(&twoe);
    ibz_init(&tmp);
    ibz_init(&r);
    ibz_init(&q);
    ibz_init(&n_gen);
    ibz_init(&fivef);
    ibz_set_from_str(&twoe, "2261564242916331941866620800950935700259179388000792266395655937654553313279", 10);
    quat_alg_init_set(&alg, &twoe);
    int e = 246;
    ibz_pow(&twoe, &ibz_const_two, e);

    // First test case, f=0
    ibz_mat_4x4_zero(&small.lattice.basis);
    ibz_set(&small.lattice.denom, 2);
    ibz_set_from_str(&small.norm, "10073331725235200955674254947912563241", 10);
    ibz_add(&small.lattice.basis[0][0], &small.norm, &small.norm);
    ibz_copy(&small.lattice.basis[1][1], &small.lattice.basis[0][0]);
    ibz_set_from_str(&small.lattice.basis[0][2], "4102749451449287101115007144975989893", 10);
    ibz_add(&small.lattice.basis[0][2], &small.lattice.basis[0][2], &small.lattice.basis[0][2]);
    ibz_set_from_str(&small.lattice.basis[1][2], "9775591589115644030266300970798841829", 10);
    ibz_set(&small.lattice.basis[2][2], 1);
    ibz_set_from_str(&small.lattice.basis[0][3], "10371071861354757881082208925026284653", 10);
    ibz_copy(&small.lattice.basis[1][3], &small.lattice.basis[0][2]);
    ibz_set(&small.lattice.basis[3][3], 1);
    ibz_set_from_str(&omega_cmp[0], "3442833116117478212", 10);
    ibz_set_from_str(&omega_cmp[1], "1558425707656217716", 10);
    ibz_neg(&omega_cmp[1], &omega_cmp[1]);

    quat_alg_elem_set(&gen, 2, 0, 0, 1, 0);
    ibz_copy(&gen.coord[0], &small.lattice.basis[0][2]);
    ibz_copy(&gen.coord[1], &small.lattice.basis[1][2]);
    quat_alg_elem_copy(&gen_cp, &gen);
    quat_qlapoty_gen_to_dim2_lattice(&Li, NULL, &gen, &small.norm);
    // ibz_swap(&Li[1][0], &Li[0][1]);
    assert(quat_dim2_lattice_contains(&Li, &omega_cmp[0], &omega_cmp[1]));
    quat_dim2_lattice_short_basis(&Li, &Li, &ibz_const_one);
    assert(quat_dim2_lattice_contains(&Li, &omega_cmp[0], &omega_cmp[1]));

    quat_alg_norm(&n_gen, &r, &gen, &alg);
    assert(ibz_is_one(&r));
    res = res || !quat_qlapoty_initialize_bounds(&ab, &omega, &gen, fbi, &small.norm, &Li, &alg, &twoe);
    res = res || !(fbi[0] == 0);
    quat_dim2_lattice_norm(&tmp, &omega[0], &omega[1], &ibz_const_one);
    quat_dim2_lattice_norm(&r, &omega_cmp[0], &omega_cmp[1], &ibz_const_one);
    res = res || !(ibz_cmp(&tmp, &r) == 0);
    res = res || !(sqisign_gen_quat_alg_elem_equal(&gen, &gen_cp));
    res = res || !quat_lattice_contains(&coeffs, &small.lattice, &gen);
    ibz_set(&fivef, 5);
    ibz_pow(&fivef, &fivef, fbi[0]);
    ibz_mul(&q, &n_gen, &fivef);
    quat_alg_norm(&tmp, &r, &gen, &alg);
    assert(ibz_is_one(&r));
    res = res || !(ibz_cmp(&tmp, &q) == 0);
    quat_qlapoty_get_short_basis(&Li, NULL, &omega, &gen, &small.norm);
    // omega  3442833116117478212 - 1558425707656217716*i
    // first_minima 1.4177822135987985
    // B1  6.44693230415053e+36
    // B2  80586653801881607645394039583300505928
    // Ba  0.4514092459768339
    // Bb  10073331725235200955674254947912563241/1785223818964871022351876303635871700
    // effective bound a, b =  0.6718699621034072 2.37541903139434
    // adjust minima
    // ell, f =  4 0
    // ell =  4
    // Ba  1
    ...
    // Zc  1
    // f  0
    // effective bound a, b =  1 2
    //-i

    // test case with f=2
    ibz_mat_4x4_zero(&small.lattice.basis);
    ibz_set(&small.lattice.denom, 2);
    ibz_set_from_str(&small.norm, "8990338725904724219837717331338424525", 10);

    ibz_add(&small.lattice.basis[0][0], &small.norm, &small.norm);
    ibz_copy(&small.lattice.basis[1][1], &small.lattice.basis[0][0]);
    ibz_set_from_str(&small.lattice.basis[0][2], "2856634425446143346270504860720231570", 10);
    ibz_add(&small.lattice.basis[0][2], &small.lattice.basis[0][2], &small.lattice.basis[0][2]);
    ibz_set_from_str(&small.lattice.basis[1][2], "7027135566849812411247230938183752689", 10);
    ibz_set(&small.lattice.basis[2][2], 1);
    ibz_set_from_str(&small.lattice.basis[0][3], "10953541884959636028428203724493096361", 10);
    ibz_copy(&small.lattice.basis[1][3], &small.lattice.basis[0][2]);
    ibz_set(&small.lattice.basis[3][3], 1);
    ibz_set_from_str(&omega_cmp[0], "2123042436988168475", 10);
    ibz_set_from_str(&omega_cmp[1], "34979166119149400", 10);
    ibz_neg(&omega_cmp[1], &omega_cmp[1]);
#ifndef NDEBUG
    quat_lattice_t test;
    quat_lattice_init(&test);
    quat_lattice_mul(&test, &small.lattice, &small.lattice, &alg);
    assert(quat_lattice_inclusion(&test, &small.lattice));
    quat_lattice_O0_set(&test);
    quat_lattice_mul(&test, &test, &small.lattice, &alg);
    assert(quat_lattice_inclusion(&test, &small.lattice));
    quat_lattice_finalize(&test);
#endif
    ...
    ibz_copy(&gen.coord[1], &small.lattice.basis[1][2]);
    quat_alg_elem_copy(&gen_cp, &gen);
    assert(quat_lattice_contains(&coeffs, &small.lattice, &gen));
    ...
    ibz_set_from_str(&omega_cmp[0], "260361759017644121", 10);
    ibz_set_from_str(&omega_cmp[1], "335489289983809028", 10);
    ...
    res = res || !(ibz_cmp(&tmp, &q) == 0);
    assert(!res);
    res = res || !(ibz_cmp(&gen.denom, &ibz_const_two) == 0);
    ibz_gcd(&tmp, &gen.coord[0], &gen.coord[1]);
    ibz_gcd(&tmp, &tmp, &small.norm);
    res = res || !ibz_is_one(&tmp);
    assert(!res);
    res = res || !quat_lattice_contains(&coeffs, &small.lattice, &gen);
    assert(!res);
    quat_qlapoty_get_short_basis(&Li, NULL, &omega, &gen, &small.norm);

    //    Fractional ideal (8990338725904724219837717331338424525,
    // 8990338725904724219837717331338424525*i,
    // 2856634425446143346270504860720231570 + 7027135566849812411247230938183752689/2*i + 1/2*j,
    // 10953541884959636028428203724493096361/2 + 2856634425446143346270504860720231570*i + 1/2*k)
    // p  [2261564242916331941866620800950935700259179388000792266395655937654553313279]
    // e  246
    // omega  2123042436988168475 - 34979166119149400*i
    // first_minima 0.5014864143354449
    // B1  5.753816784579024e+36
    // B2  71922709807237793758701738650707396200
    // Ba  1.2762060580406935
    // Bb  2876908392289511750348069546028295848/180341309252602094358331380580167425
    // effective bound a, b =  1.1296929043066057 3.9940675664676317
    // adjust minima
    // ell, f =  25 2
    // ell =  1
    // Ba  36
    // Bb  361
    // omega  260361759017644121 + 335489289983809028*i
    // Zc  3 + 4*i
    // f  2
    // effective bound a, b =  6 19
    //-8 + 5*i

    if (res) {
        printf("Quaternion unit test qlapoty_initialize_bounds failed\n");
    }

    ibz_vec_4_finalize(&coeffs);
    ibz_vec_2_finalize(&ab);
    ibz_vec_2_finalize(&omega);
    ibz_vec_2_finalize(&omega_cmp);
    ibz_mat_2x2_finalize(&Li);
    quat_left_ideal_finalize(&small);
    quat_alg_elem_finalize(&gen);
    quat_alg_elem_finalize(&gen_cp);
    ibz_finalize(&twoe);
    quat_alg_finalize(&alg);
    ibz_finalize(&tmp);
    ibz_finalize(&r);
...
quat_test_qlapoty_qlapoty()
{
    ...
    output = quat_qlapoty(&mu1, &mu2, &theta, &small, &lideal, &alg, 20000, 17, 246, &cornacchia_params);
    res = res || !output;
    ...
    if (!res) {
        ...
        quat_lideal_mul(&I2, &small_equiv, &mu2, &alg);
        ibz_add(&n, &I1.norm, &I2.norm);
        ibz_pow(&cofactor, &ibz_const_two, 246);
        res = res || !(ibz_cmp(&cofactor, &n) == 0);
...
quat_test_qlapoti(void)
{
    ...
    res = res | quat_test_qlapoty_initialize_bounds();
    res = res | quat_test_qlapoty_qlapoty();
    res = res | quat_test_qlapoty_randomized_qlapoty();
    res = res | quat_test_gaussian_mul();
    res = res | quat_test_gaussian_gcd();
    return (res);
}
